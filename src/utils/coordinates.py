...
DEFAULT_COORDINATES: dict[str, Tuple[float, float]] = {
    # 微信主界面（(0,0) = 未配置，需引导校准）
    "tab_chat":          (0.0, 0.0),   # 聊天标签 — 左栏导航第1个图标（未配置）
    "tab_contacts":      (0.0, 0.0),   # 通讯录标签 — 左栏导航第3个图标（未配置）
    "btn_contacts_mgr":  (0.0, 0.0),   # 通讯录管理按钮 — 通讯录页面顶部（未配置）
    "chat_first":        (0.0, 0.0),   # 聊天区域点击 — 聊天列表第一个聊天（未配置）
    "chat_dismiss":      (0.55, 0.50), # 聊天区域点击(关闭弹窗) — 窗口中央偏右

    # 通讯录管理窗口（全屏后）
    "cm_search_box":     (0.0, 0.0),   # 搜索框 — 顶部搜索输入框（未配置）
    "cm_list_focus":     (0.0, 0.0),   # 列表聚焦点 — 联系人列表中间位置（未配置）
